...
class DataScheduler():
    ...
    def eval(self, model, writer, step, eval_title, task):
        for eval_dataset in self.eval_datasets.values():
            eval_dataset.eval(model, writer, step, eval_title, task)
...
class BaseDataset(Dataset, ABC):
    ...
    def eval(self, model, writer: SummaryWriter, step, eval_title, task):
        if self.config['eval']:
            self._eval_model(model, writer, step, eval_title, task)

    @abstractmethod
    def _eval_model(self, model, writer: SummaryWriter, step, eval_title, task):
...
class ClassificationDataset(BaseDataset):
    ...
    def _eval_model(self, model, writer: SummaryWriter, step, eval_title, task):
        ...
        test_class = []
        for subset in task:
            for content in subset['subsets']:
                test_class.append(content[1])

        for subset_name, subset in self.subsets.items():
            if subset_name not in test_class:
                continue
