...
class SPR(torch.nn.Module):
    ...
    def learn(self, x, y, corrupt, idx, step=None):
        ...
        for i in range(len(x)):
            ...
            if self.num_updates<self.config['delayed_buffer_size']:
                continue
    ...
    def train_self_base(self, sample_num):
        ...
        expert_bs = len(self.delay_buffer)

        self.expert.train()
        self.base.train()
        self.expert.init_ntxent(self.config, batch_size=expert_bs)

        total_contra_loss, total_loss, correct, num_data = 0.0, 0.0, 0.0, 0.0
        ...
        for i in range(self.config['expert_train_epochs']):
            ...
            #print("expert_bs", expert_bs)
            replay_data = self.delay_buffer.sample(num = expert_bs)
            x = replay_data['imgs']
        ...
        print("base_train_epoch", self.config['base_train_epochs'])
        ...
        #self.report_training(sample_num, total_contra_loss / self.config['expert_train_epochs'], 0, True)
        self.writer.flush()
        self.base_step += self.config['base_train_epochs'] # * len(dataloader)
        self.expert_step += self.config['base_train_epochs'] #* len(dataloader)
