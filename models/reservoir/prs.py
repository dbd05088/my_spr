...
from my_augment import Kornia_Randaugment
import kornia
...
class PRS(rsvrBase):
    ...
    def get_batch(self):
        whole_list = []
        for i in self.substreams._data.keys():
            substream = self.substreams._data[i]
            for j in substream.idxs:
                whole_list.append((i, j))
        #print("whole_list")
        #print(whole_list)
    ...
    def update(self, imgs, cats, **other_datas):
        ...
        for s_i in range(nbatch):
            ...
            if keys not in self.learned_keys:
                self.learned_keys.append(keys)
    ...
    def memory_statistics(self):
        ...
        print(self.learned_keys)
        for key in self.learned_keys:
            count.append(len(self.substreams[key[0]]))
        print("****************")
        print(count)
        print("****************")
    ...
    def sample(self, num, cat=None):
        ...
        if cat is None:
            '''
            corrupts = self.rsvr["corrupts"]
            corrupts_list = list(filter(lambda e:corrupts[e] == 0, range(len(self))))
            #rsvr_idxs = np.random.choice(range(len(self)), num)
            if len(corrupts_list) <= num:
                rsvr_idxs = np.array(corrupts_list)
            else:
                rsvr_idxs = np.random.choice(range(len(corrupts_list)), num)
                rsvr_idxs = np.array(corrupts_list)[rsvr_idxs]
            '''
            rsvr_idxs = np.random.choice(range(len(self)), num)
        else:
        ...
        self.transform= nn.Sequential(
                    kornia.augmentation.RandomResizedCrop(size=(224,224)),
        ...
        for dtype in self.rsvr.keys():
            '''
            if dtype == "idxs":
                print("idxs", self.rsvr[dtype][rsvr_idxs])
            '''
            if isinstance(self.rsvr[dtype], torch.Tensor):
                '''
                if dtype == "imgs":
                    samples[dtype] = self.transform(self.rsvr[dtype][rsvr_idxs])
                else:
        ...
        if self.is_slab:
            samples['cats'] = torch.Tensor([int(self._onehot_to_slab(cats).item()) for cats in samples['cats']]).to(self.device)
